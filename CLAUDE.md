...
- **MockTimers ids restart at 1 in every mock session.** A `clearTimeout` on a handle left over from an earlier test's session removes an *unrelated* timer in the current one. `app.js`'s module-scope `syncTimer` survives between tests in `apply.test.js`, so this is live there.
...
- **The app has an EDIT PATH: the To-do checkbox (`done`, V6) and the item sheet (title, date, time, endTime, type, notes, via `app.js`'s `editItem`).** Every write bumps `updatedAt`, deliberately: `unionById`'s ties go to remote, so a write that did not bump would be silently reverted by the next sync. User writes (edit, Undo, tick, delete commit) are stamped strictly later than the record they replace (`js/edit.js`'s `nextStamp`), so a single write cannot lose to clock skew; concurrent writes still can. The price is whole-record last-write-wins — the same item edited on both devices inside one sync window loses the older write whole — and that `applyTombstones` can resurrect an item deleted on another device before the edit synced (reappearing and re-deletable, not lost data). Edits send only the changed fields; `deletedAt` is when the delete *commits* (Undo toast expiry or backgrounding), not the tap. Nothing changed in `merge()`, the tombstone kinds or `schemaVersion`. `js/merge.js`'s header carries spec § 5 in full and `tests/editsync.test.js` pins it on field values. **What re-opens it:** a field edited concurrently often, or content where last-write-wins loses meaningful text — that argues for per-field timestamps and a `schemaVersion` bump, not a patch to `unionById`.
